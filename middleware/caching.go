...
// Logger is a minimal logging interface used by middleware.
// It mirrors the subset of forge.Logger that caching needs,
// avoiding a direct import of the root bastion package.
...
// CacheStore is the interface for a cache backend.
...
// Route is the minimal route representation needed by the cache middleware.
// The full Route type lives in the root bastion package.
type Route struct {
	Path    string
...
func NewResponseCache(config CachingConfig, logger Logger, store CacheStore) *ResponseCache {
	rc := &ResponseCache{
		config:  config,
		logger:  logger,
		store:   store,
