...
func TestNewLoadBalancer(t *testing.T) {
	strategies := []bastion.LoadBalanceStrategy{
		bastion.LBRoundRobin,
		bastion.LBWeightedRoundRobin,
		bastion.LBRandom,
		bastion.LBLeastConnections,
		bastion.LBConsistentHash,
	}
...
func TestLoadBalancer_RoundRobin(t *testing.T) {
	lb := NewLoadBalancer(bastion.LBRoundRobin)
...
func TestLoadBalancer_WeightedRoundRobin(t *testing.T) {
	lb := NewLoadBalancer(bastion.LBWeightedRoundRobin)
...
func TestLoadBalancer_Random(t *testing.T) {
	lb := NewLoadBalancer(bastion.LBRandom)
...
func TestLoadBalancer_LeastConnections(t *testing.T) {
	lb := NewLoadBalancer(bastion.LBLeastConnections)

	t1 := &bastion.Target{ID: "t1", URL: "http://localhost:8080", Healthy: true}
	t2 := &bastion.Target{ID: "t2", URL: "http://localhost:8081", Healthy: true}
	t3 := &bastion.Target{ID: "t3", URL: "http://localhost:8082", Healthy: true}
	...
	// Snapshot syncs the private atomic counters into the exported ActiveConns
	// field that the routing package's load balancer reads.
	t1.Snapshot()
	t2.Snapshot()
	t3.Snapshot()

	targets := []*bastion.Target{t1, t2, t3}
...
func TestLoadBalancer_ConsistentHash(t *testing.T) {
	lb := NewLoadBalancer(bastion.LBConsistentHash)
