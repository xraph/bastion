...
//   - gRPC trailers
//   - gRPC status codes
func ProxyGRPC(w http.ResponseWriter, r *http.Request, route *bastion.Route, target *bastion.Target, config bastion.Config, logger forge.Logger) {
	proxy := NewGRPCProxy(config, logger)
	proxy.ServeHTTP(w, r, route, target)
}

// ServeHTTP processes a gRPC proxy request.
func (gp *GRPCProxy) ServeHTTP(w http.ResponseWriter, r *http.Request, route *bastion.Route, target *bastion.Target) {
	start := time.Now()
