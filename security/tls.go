...
func (tm *TLSManager) TargetTLSConfig(target Target) *tls.Config {
	targetTLS := target.GetTLS()
	if targetTLS == nil || !targetTLS.Enabled {
	...
	// Check cache first
	if cached, ok := tm.targetTLS.Load(targetID); ok {
		return cached.(*tls.Config)
	}

	// Build per-target TLS config
	cfg := tm.buildTargetTLSConfig(targetTLS)
	tm.targetTLS.Store(targetID, cfg)
...
// Stop stops the certificate reload loop. It is safe to call multiple times.
